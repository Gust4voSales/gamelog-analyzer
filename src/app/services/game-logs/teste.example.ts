...
import { ProcessGameLogsService } from './process-game-logs-service';
...
export function demonstrateParser(): void {
  const processGameLogsService = new ProcessGameLogsService(new MatchesLogsParser())

  processGameLogsService.execute(exampleLog)
}
